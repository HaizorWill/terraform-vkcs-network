locals {
  private_dns_domain = endswith(var.private_dns_domain, ".") ? var.private_dns_domain : format("${var.private_dns_domain}%s", ".")
  subnets = var.subnets != null ? { for id, subnet in var.subnets :
    coalesce(subnet.resource_key, subnet.name, format("subnet-%s", id)) => merge(subnet, {
      # no_gateway         = subnet.gateway_ip != null ? false : subnet.no_gateway
